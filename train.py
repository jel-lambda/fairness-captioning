...
def main(args):
    ...
    elif args.decoder_training:
        optimizer = optim.Adam(decoder.parameters(), lr=args.lr)
    elif args.encoder_training:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train(epoch, encoder, decoder, optimizer, optimizer_nce, optimizer_club, cross_entropy_loss_train, infoNCE, club,
              train_loader, word_dict, args.log_interval, writer, args)
...
def train(epoch, encoder, decoder, optimizer, optimizer_nce, optimizer_club, cross_entropy_loss, infoNCE, club, data_loader, word_dict, log_interval, writer, args):
    if args.encoder_training:
        encoder.train()
    else:
        encoder.eval()
    ...
    for batch_idx, ((img1, cap1), (img2, cap2), club_mask, nce_mask) in enumerate(data_loader):
        ...
        if args.use_filter:
            ...
            loss = ce_loss / total_caption_length
...
def validate(epoch, encoder, decoder, cross_entropy_loss, data_loader, infoNCE_loss, club_loss, word_dict, alpha_c, log_interval, writer):
    ...
    with torch.no_grad():
        for batch_idx, ((img1, cap1, all_cap1), (img2, cap2, all_cap2), club_mask, nce_mask) in enumerate(data_loader):
            ...
            c_loss = club_loss(features1, features2, club_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-filter', action='store_true', default=False,
                        help='Decide using filter or not (default: True)')
    parser.add_argument('--encoder-training', action='store_true', default=False,
                        help='Decide training on encoder or not (default: False)')
    parser.add_argument('--decoder-training', action='store_true', default=False,
                        help='Decide training on decoder or not (default: False)')
